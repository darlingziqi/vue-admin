...
export default {
  ...
  methods: {
    ...
    */
    updateRole(role) {
      this.$API.role
        .default.updateById({ id: role.id, roleName: role.roleName })
        .then((result) => {
    ...
    */
    addRole() {
      ...
      this.$prompt("请输入新名称", "添加角色", {
        ...
        .then(({ value }) => {
          this.$API.role.default.save({ roleName: value }).then((result) => {
    ...
    */
    getRoles(page = 1) {
      ...
      this.$API.role
        .default.getPageList(page, limit, searchObj)
        .then((result) => {
    ...
    */
    removeRole({ id, roleName }) {
      this.$confirm(`确定删除 '${roleName}' 吗?`, "提示", {
        ...
        .then(async () => {
          const result = await this.$API.role.default.removeById(id);
    ...
    removeRoles() {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          ...
          const result = await this.$API.role.default.removeRoles(ids);
          this.getRoles();
          this.$message({
            type: "success",
            message: "批量删除成功!",
